...
import { Card, Grid, Pagination, Box, TextField, Typography } from "@mui/material"
...
export default function UserUsageTable() {
  ...
  const chunkSize = 10
  const splitRows = [paginatedRows.slice(0, chunkSize), paginatedRows.slice(chunkSize, chunkSize * 2), paginatedRows.slice(chunkSize * 2, chunkSize * 3)]
  ...
  return (
    <Card sx={{ width: "70%", margin: "20px auto", boxShadow: 3, padding: 2 }}>
      ...
      <Box display="flex" justifyContent="center" alignItems="center" mt={1}>
        <Pagination count={totalPages} page={currentPage + 1} onChange={(_, page) => setCurrentPage(page - 1)} color="primary" />
      </Box>
    </Card>
  )
}
